...
class new_slider:
    ...
    def __init__(self, window, row, column, min, max, padx, pady, id_number, text, move_function):
        self.window = window
        self.row = row
        self.column = column
        self.min = min
        ...
        self.slider.bind('<ButtonRelease-1>', self.on_slider_value_changed)  # Wywołanie po zwolnieniu przycisku

    def select_slider(self, event):
        new_slider.selected_slider = self
        self.update_label_from_slider(None)

    def move_slider_left(self, event):
        if new_slider.selected_slider:
            current_value = new_slider.selected_slider.slider.get()
            new_value = max(current_value - 50, new_slider.selected_slider.min)
            new_slider.selected_slider.slider.set(new_value)
            new_slider.selected_slider.update_label_from_slider(None)

    def move_slider_right(self, event):
        if new_slider.selected_slider:
            current_value = new_slider.selected_slider.slider.get()
            new_value = min(current_value + 50, new_slider.selected_slider.max)
            new_slider.selected_slider.slider.set(new_value)
            new_slider.selected_slider.update_label_from_slider(None)
    ...
    def on_slider_value_changed(self, event):
        if self.move_function is 1:
            if new_slider.selected_slider:
                id_number = new_slider.selected_slider.id_number
                position = new_slider.selected_slider.slider.get()
