...
class GUI:
    def __init__(self, root) -> None:
        ...
        self.btn_connection = ctk.StringVar()
        self.com_ports= ctk.StringVar()

        self.combo = ctk.CTkComboBox(root, variable=self.com_ports)
        self.btn_refresh_conn = ctk.CTkButton(root, textvariable=self.btn_connection, fg_color='red')
        self.btn_refresh_com = ctk.CTkButton(root, text="Refresh COM Ports")
        self.btn_compute = ctk.CTkButton(root, text="Compute")

        self.entry_x = ctk.CTkEntry(root, width=200)
        self.entry_y = ctk.CTkEntry(root, width=200)
        self.entry_z = ctk.CTkEntry(root, width=200)
        self.label_cords = ctk.CTkLabel(root, text="End-Effector Coordinates:\nX: 0.00, Y: 0.00, Z: 0.00")

        # table
        self.tree_columns = ("Id", "Voltage", "Current", "Temperature", "Position", "Load")
        self.tree = ttk.Treeview(root, columns=self.tree_columns, show="headings")
    ...
    def init(self, root):
        ...
        self.btn_refresh_conn.grid(row=3, column=1, padx=10, pady=10)
        self.btn_refresh_com.grid(row=2, column=1, padx=10, pady=10, sticky="w")
        ...
        self.btn_compute.grid(row=4, column=0, padx=50, pady=10, sticky="w")
        self.label_cords.grid(row=5, column=0, columnspan=2, padx=50, pady=10, sticky="w")
    ...
    def plot_robot(self, theta1, theta2, theta3, theta4):
        ...
        self.label_cords.configure(text=f"End-Effector Coordinates:\nX: {self.robot.rx:.2f}, Y: {self.robot.ry:.2f}, Z: {self.robot.rz:.2f}")
